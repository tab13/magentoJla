    ...
    public function dispatch()
    {
        ...
        switch ($this->getActionType()) {
            ...
            case self::ACTION_SAVE_SHIPPING_METHOD:
                ...
                if ($method == 'webpos_shipping_cs1') {
                    $customShipping1Price = floatval($this->_processRequestParams('custom_method_1price'));
                    if ($customShipping1Price && $customShipping1Price >= 0) {
                        $session->setData('custom_method_1price', $customShipping1Price);
                    }
                }
            ...
            case self::ACTION_PLACE_ORDER:
                ...
                // CUSTOMIZE edit custom method shipping US13
                Mage::getModel('core/session')->setData('custom_method_1price', Mage::helper('webpos')->getStoreConfig('carriers/webpos_shipping/cs1_price'));
                // CUSTOMIZE edit custom method shipping end US13
                $session = Mage::helper('webpos/permission')->getCurrentSessionModel();
                if ($session->getGiftcodes()||$session->getRewardSalesRules()){
                    $session->setGiftcodes(null)
                        ->setRewardSalesRules(null)
