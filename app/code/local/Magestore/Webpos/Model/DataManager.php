...
class Magestore_Webpos_Model_DataManager {
    ...
    const WEBPOS_LOCATION_ADDRESS = 'webpos_location_address';
    ...
    const USE_CUSTOM_ORDER_ID = 'use_custom_order_id';
    const CUSTOM_ORDER_ID_PREFIX = 'custom_order_id_prefix';
    ...
    public function getWebposData($session = false){
        $useMagentoOrderIncrementId = $this->_helper->getStoreConfig('webpos/general/use_magento_order_increment_id');
        $customOrderIdPrefix = $this->_helper->getStoreConfig('webpos/general/custom_order_id_prefix');
        $quote = Mage::getModel('sales/quote');
        $session = ($session)?$session:$this->_helperPermission->getCurrentSession();
        $storeId = null;
        // CUSTOMIZE edit custom method shipping US13
        $customMethod1Price = $this->_helper->getStoreConfig('carriers/webpos_shipping/cs1_price');
        ...
        if($session){
            ...
            $location = $this->_helperPermission->getCurrentLocationObject();
            if($quoteId){
            ...
            if($location){
                $customOrderIdPrefix = ($location->getUseGlobalPrefix())?$customOrderIdPrefix:$location->getCustomOrderPrefix();
            }
        }
        $defaultCustomer = $this->_helperConfig->getDefaultCustomer($storeId);
        $quoteData = array(
            ...
            // CUSTOMIZE edit custom method shipping US13
            Magestore_Webpos_Api_Cart_QuoteDataInitInterface::CUSTOM_METHOD_1PRICE => ($quote->getId())?$customMethod1Price:0
            // CUSTOMIZE edit custom method shipping end US13
        );
        ...
        $data = array(
            self::USE_CUSTOM_ORDER_ID => ($useMagentoOrderIncrementId)?false:true,
            self::CUSTOM_ORDER_ID_PREFIX => $customOrderIdPrefix,
            ...
            // CUSTOMIZE edit custom method shipping US13
            Magestore_Webpos_Api_Cart_QuoteDataInitInterface::CUSTOM_METHOD_1PRICE => $quoteData[Magestore_Webpos_Api_Cart_QuoteDataInitInterface::CUSTOM_METHOD_1PRICE]
            // CUSTOMIZE edit custom method shipping end US13
        );
        return $data;
    }
