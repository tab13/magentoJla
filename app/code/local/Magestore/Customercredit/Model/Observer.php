...
class Magestore_Customercredit_Model_Observer
{
    ...
    public function adminSystemConfigChangedCustomercredit($observer) {
//        \Zend_Debug::dump($observer->getRequest());
//        die();
    }

    public function sendNotifyExpirationDateEmail() {
    ...
    public function customerSaveAfter($observer)
    {
        ...
        try {
            ...
            // CUSTOMIZE store credit US03
            if ($sign != "-") {
                $returnCustomer = Mage::helper('customercredit')->setCreditExpirationDate($customer->getId(), $credithistory->getData('transaction_id', false));
                if (!is_null($returnCustomer->getCreditExpirationDate())) {
                    Mage::helper('customercredit')->setCreditExpirationNotifyDate($customer->getId());
                }
            }
            // CUSTOMIZE store credit end US03
        } catch (Mage_Core_Exception $e) {
