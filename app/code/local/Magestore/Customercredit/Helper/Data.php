...
class Magestore_Customercredit_Helper_Data extends Mage_Core_Helper_Data
{
     ...
     * @return mixed
     */
    public function getExpirationDateConfig($code, $store = null)
    {
        return Mage::getStoreConfig('customercredit/expiration_date/' . $code, $store);
    }

    public function getCustomerCreditExpirationDate($customerId)
    {
        $customercredit = Mage::getModel('customer/customer')->load($customerId);
        $expirationDate = '';
        if ($customercredit->getCreditExpirationDate()) {
            $expirationDate = date('d/m/Y',strtotime($customercredit->getCreditExpirationDate()));
        }
        return $expirationDate;
    }

    public function setCreditExpirationDate($customerId, $transactionId) {
        $customerModel = Mage::getModel('customer/customer')->load($customerId);
        $transactionModel = Mage::getModel('customercredit/transaction')->load($transactionId);
        if ($transactionModel) {
            $dateExpire = date('Y-m-d H:i:s', strtotime($transactionModel->getTransactionTime()  . ' +' . $this->getExpirationDateConfig('expiration_period')  . ' day'));
    ...
    public function setCreditExpirationDateAfterChangeConfig($old, $new) {
        ...
        foreach ($customers as $customer) {
            ...
            if (!is_null($customerExpireDate)) {
                ...
                $customercredit->setCreditExpirationDate($newExpireDate)->save();
            }
        }
    }
    ...
    public function setCreditExpirationNotifyDate($customerId) {
        $customerModel = Mage::getModel('customer/customer')->load($customerId);
        $customerExpireDate = $customerModel->getCreditExpirationDate();
        $notifyDays = unserialize($this->getExpirationDateConfig('notify_customer'));
        ...
        foreach ($notifyDays as $key => $value ) {
            ...
            array_push($notifyDatesArray,$notifyDate);
            if (!$isSendEmail && strtotime($notifyDate) <= strtotime(now())) {
                Mage::getModel('customercredit/customercredit')->sendNotifyExpirationDateEmail($customerId);
                $isSendEmail = true;
            }
        }
