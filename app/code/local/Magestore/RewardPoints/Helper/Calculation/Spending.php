...
class Magestore_RewardPoints_Helper_Calculation_Spending extends Magestore_RewardPoints_Helper_Calculation_Abstract {
    ...
    public function getQuoteBaseTotal($quote, $address = null) {
        ...
        $isTaxAfterDiscount = Mage::getStoreConfig('tax/calculation/apply_after_discount');
        $baseTotal = 0;
        foreach ($address->getAllItems() as $item) {
            ...
            if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                foreach ($item->getChildren() as $child) {
                    if ($isTaxAfterDiscount){
                        $baseTotal += $child->getBaseRowTotal() * (1 - $child->getBaseDiscountAmount() / $child->getBaseRowTotalInclTax());
                    } else {
                ...
                if ($isTaxAfterDiscount){
                    $baseTotal += $item->getBaseRowTotal() * (1 - $item->getBaseDiscountAmount() / $item->getBaseRowTotalInclTax());
                } else {
        ...
        if (Mage::getStoreConfig(self::XML_PATH_SPEND_FOR_SHIPPING, $quote->getStoreId())) {
            ...
            // CUSTOMIZE store credit US12
            if ($address->getBaseSurcharge() > 0) {
                $baseShippingAmount += $address->getBaseSurcharge();
            }
            // CUSTOMIZE store credit end US12
