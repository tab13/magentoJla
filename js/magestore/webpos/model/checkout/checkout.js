...
define(
    ...
    function ($, ko, __, CartModel, DiscountModel, HelperPrice, OrderHelper, Staff, DateTime, Event, Totals, Helper,
              Alert, OrderFactory, CheckoutResource, CreditCard, DataManager, AddNoti, DirectPost, PaynlInstore, moment, Cookies,
              ShoppingCartModel
    ) {
        "use strict";
        ...
        var CheckoutModel = {
            ...
            saveShipping: function (data) {
                ...
                if (Helper.isOnlineCheckout() && data.code) {
                    ...
                    self.saveShippingMethodOnline(data.code, data.price);
            ...
            saveShippingAddress: function (data) {
                ...
                if (data.id == 0) {
                    ...
                    var shippingAddress = $.extend({}, data);
                    var shippingMethod = self.selectedShippingCode();
                    if (shippingMethod == 'webpos_shipping_storepickup') {
            ...
            saveShippingMethodOnline: function (code, price) {
                var deferred = $.Deferred();
                if (code) {
                    var self = this;
                    var params = CartModel.getQuoteInitParams();
                    ...
                    // CUSTOMIZE edit custom method shipping US13
                    if (params.shipping_method === 'webpos_shipping_cs1') {
                        params.custom_method_1price = price;
                    }
                    // CUSTOMIZE edit custom method shipping end US13
                    self.loading(true);
